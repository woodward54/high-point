...
public class HexGrid : Singleton<HexGrid>
{
    ...
    public int InstantiatedCellCount { get; private set; }
    ...
    void FinalizeTerrain()
    {
        ...
        // Remove neighbors with null terrain
        foreach (var cell in _cells.Values)
        {
            cell.RemoveEmptyNeighbors();
        }
    ...
    public bool IsPathPossible(HexCell a, HexCell b)
    {
        var aNode = AstarPath.active.GetNearest(a.Terrain.transform.position).node;
        var bNode = AstarPath.active.GetNearest(b.Terrain.transform.position).node;

        return PathUtilities.IsPathPossible(aNode, bNode);
    }
    ...
    {
        return _cells.Values.Where(c => c.Terrain != null).ToList();
    }

    public HexCell GetCell(Vector3Int coords)
    {
        return GetCell(coords.x, coords.z);
    }

    public HexCell GetCell(Vector2Int coords) 
    {
        if (_cells.TryGetValue(coords, out var hexCell))
        {
            return hexCell;
        }
        else
    ...
    private void ValidateSettings()
    {
        ...
        var remainder = (int)Math.Round(YStepSize * 1000) % (int)Math.Round(HexSize * 1000);
        Debug.Assert(YStepSize >= HexSize, "YStepSize must be greater than HexSize");
        Debug.Assert(remainder == 0, "YStepSize must be a multiple of HexSize");
    }
