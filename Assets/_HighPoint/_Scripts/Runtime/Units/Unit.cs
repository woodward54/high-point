...
public abstract class Unit : MonoBehaviourGizmos, IDamageable
{
    ...
    protected MMF_Player _floatingTextPlayer;

    protected float AttackRangeSquared => AttackRange * AttackRange;
    ...
    protected virtual void Update()
    {
        ...
        // TODO: .ToString() is slow
        _activeState = FSM.ActiveStateName.ToString();
        _isDead = IsDead;
#endif
    }

    public void TakeDamage(float damage, Unit attackingUnit, float delay = 0f)
    {
        if (IsDead) return;
    ...
    protected virtual void UpdateHealth(float damage)
    {
    ...
    {
        Vector3 offset = HexGrid.Instance.HexSize * UnitConfig.HealthBarOffset * 1.2f *
                            Vector3.up;

        var floatText = _floatingTextPlayer.GetFeedbackOfType<MMF_FloatingText>();
        floatText.Value = msg;
        _floatingTextPlayer.PlayFeedbacks(transform.position + offset);
    }
    ...
    public void Setup(MMF_Player floatingTextPlayer)
    {
        _floatingTextPlayer = floatingTextPlayer;
    }
